...
extern uart_rtos_handle_t wifi_handle;
extern struct _uart_handle wifi_t_handle;
extern uint8_t background_buffer[UART_RX_BUF_SIZE];
uint8_t wifi_recv_buffer[1024];
int wifi_produce_init()
{
	...
	uart_wifi_sendData(NULL, 0, UART_CMD_iwlist_scanning);

	return 1;
}

int wifi_produce_check_ack()
{
	size_t 	n;

	n = UART_RTOS_Receive_NoBlocking_Safe(&wifi_handle, wifi_recv_buffer, sizeof(wifi_recv_buffer));
	if (0 == n){
		vTaskDelay(3000);
		n = UART_RTOS_Receive_NoBlocking_Safe(&wifi_handle, wifi_recv_buffer, sizeof(wifi_recv_buffer));
	}
