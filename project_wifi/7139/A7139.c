...
void send_preamble_tx_test(void)
{
	uint8_t vpal[64];
	for(int i=0;i<64;i++)
	vpal[i]=0x55;
...
//	A7139_WritePageA(CKO_PAGEA, 0xD283);    //CKO=DCK
	...
	A7139_WritePageB(TX2_PAGEB, 0xD30F);  //10DB
...
int SPI_A7139_TX_TEST(uint32_t freq)
{
	uint32_t ID = 0x78877887;
	A7139_Init(freq);
    A7139_SetCID(ID);
    if (ID != A7139_GetCID()) return -1;

    //send_preamble_tx_test();
    A7139_Direct();
    //0xD30F
    vTaskDelay(2500);
