...
//volatile uint32_t send_stat=0;
uint32_t send_plc_msg(uint8_t *pBuf, uint32_t length)
{
	uint32_t bytesToSend = length;
	uint32_t noRspCount = 0;

//	send_stat++;
//	PRINTF("PLC SEND FUNC %d\n", send_stat);
    while (bytesToSend)
    {
    	PRINTF("PLC bytesToSend %d\n", bytesToSend);
    	if (bytesToSend > MAX_PLC_DATA_LEN){
			if(Drv_DataRequest(NULL, pBuf, MAX_PLC_DATA_LEN, APP_TxEnd) == ERR_SUCCESS){
				g_tTest.tCntr.nTxCntr++;
				bytesToSend -= MAX_PLC_DATA_LEN;
				pBuf += MAX_PLC_DATA_LEN;
			}
			else{
				PRINTF("PLC NO response\n");
				noRspCount++;
			}
    	}else{
			if(Drv_DataRequest(NULL, pBuf, bytesToSend, APP_TxEnd) == ERR_SUCCESS){
				g_tTest.tCntr.nTxCntr++;
				bytesToSend = 0;
			}
			else{
...
{      
    ...
    rev_data.len = nLen;
    if (rev_data.len>248) rev_data.len = 248;

	memcpy(&rev_data.plc_data, pBuf, nLen);
    xQueueSend(plc_data_queue, ( void * )&rev_data, ( TickType_t ) 10U );
