...
void audio_feed_task(void * pvParameters)
{
	...
	while(1)
	{
		...
		msg_audio_switch = get_hb_msg_audio_switch();
		printf("msg_audio_switch:%d\r\n",msg_audio_switch);

		if( AUDIO_SOS_CALL == audio_cmd )
		{
			printf("AUDIO_SOS_CALL...\r\n");
			audio_read_num = AUDIO_SOS_START_BLOCK;
			audio_max_num = AUDIO_SOS_END_BLOCK;
		}
		else if((AUDIO_ALARM_MAG == audio_cmd) || (AUDIO_ALARM_WATER == audio_cmd)
			|| (AUDIO_ALARM_PIR == audio_cmd) ||(AUDIO_ALARM_SMOKE == audio_cmd))
		{
			if ( AUDIO_ALARM_ON != msg_audio_switch )
			{
				printf("AUDIO_SENSOR_ALARM FAIL...\r\n");
				audio_max_num = AUDIO_FAIL_BLOCK;
			}
			else
			{
				printf("AUDIO_SENSOR_ALARM...\r\n");
				audio_read_num = AUDIO_ALARM_START_BLOCK;
				audio_max_num =  AUDIO_ALARM_END_BLOCK;
			}
		}
		else
		{
			printf("AUDIO CMD ERROR...\r\n");
			audio_max_num = AUDIO_FAIL_BLOCK;
		}
		...
		while(1)
		{
			...
			if (audio_read_num >= audio_max_num) {
				PIT_StopTimer(PIT, PIT_CHANNEL);
				memset(Audio_Source_Blk_A,0xFF,sizeof(Audio_Source_Blk_A));
				memset(Audio_Source_Blk_B,0xFF,sizeof(Audio_Source_Blk_B));
				break;
			}
