...
int FreeRTOS_read(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	do
	{
		...
		FreeRTOS_setsockopt(n->my_socket, FREERTOS_SOL_SOCKET, FREERTOS_SO_RCVTIMEO, &xTicksToWait, sizeof(xTicksToWait));
...
int FreeRTOS_write(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	do
	{
		...
		FreeRTOS_setsockopt(n->my_socket, FREERTOS_SOL_SOCKET, FRERRTOS_SO_SNDTIMEO, &xTicksToWait, sizeof(xTicksToWait));
	...
	//	FreeRTOS_setsockopt(n->my_socket, FREERTOS_SOL_SOCKET, SO_SNDBUF, &sndlen, sizeof(sndlen));
		...
		FreeRTOS_getsockopt(n->my_socket, FREERTOS_SOL_SOCKET, FRERRTOS_SO_ERROR, &err, &errlen);
		printf("err :%d, rc :%d\r\n",err,rc);
		if (rc > 0)
			sentLen += rc;
		else if (rc < 0)
...
#else
	n->my_socket = 0;
	n->mqttread = wifi_mqtt_netconn_read;
	n->mqttwrite = wifi_mqtt_netconn_write;
	n->disconnect = wifi_mqtt_netconn_close;
#endif
}
...
#else
	req_ack_type ack_type;
	ack_type.send_cmd = UART_MQTT_CON_SERVER;
	ack_type.recv_cmd = UART_MQTT_CON_SERVER_ACK;
	if(wifi_mqtt_connect_to_server(addr, port, &ack_type, 10000) == 0)
		retVal = 0;
#endif
exit:
	return retVal;
