...
void init_A7139()
{
    ...
    A7139_Init(875.001);
    A7139_SetCID(0x78877887);//0x78877887,0x12345678
    ID = A7139_GetCID();
    PRINTF("0x%x \r\n", ID);
...
void rf_send_message_no_preamble(void* msg, uint32_t length)
{
	...
	A7139_WriteFIFO(msg, length);
	A7139_SetGIO2_Func(e_GIOS_WTR);
	A7139_SetPackLen(length);//length
	GIO2S = TXIN;
	GIO2_FLAG = 0;
	A7139_StrobeCmd(CMD_TX);
}
