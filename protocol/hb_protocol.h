...
#ifdef BRIDGE
#define BRIDGE_ID 0x05555555
#else



#define HBB_ID	0x00206005
#define HBB_LONG_IDH	0x01001000
#define HBB_LONG_IDM	0x017F26A8
...
#define PACKET_CONTENT_LENGTH 23
#define PACKET_CRC_LENGTH 18


#define SENSOR_POLLING_INTERVAL       3600000
#define SENSOR_ALARM_POLLING_INTERVAL 10000
#define SENSOR_ALARM_POLLING_INTERVAL_L 60000

#define WDOG_RESET_INTERVAL 30000

#ifdef BRIDGE
#define WAIT_SENSOR_LIST_TIMEOUT 2000
#else
#define WAIT_SENSOR_LIST_TIMEOUT 180000
#endif
...
extern HBB_STATUS_STORAGE_BLOCK hbb_info;
extern plc_bridge_node bridge_list[];


__inline__ int get_alarm_status(int index)
{
...
uint32_t rf_send_image_norflash();
...
void get_hbb_info();
int install_sensor(LONG_ID96 *longid, uint32_t dt);
int get_sensor_type_handle(SENSOR_STATUS_STORAGE_BLOCK *status, uint32_t sid);
typedef void (*rf_callback_fn)(RF_MSG *msg);
err_t hb_do_rf_send_callback(uint32_t sid, RF_MSG *msg);
...
int send_uplink_event_core(LONG_ID96 *id, uint32_t dt, uint32_t status, uint32_t fwv, uint32_t hwv, int p, char *str);
...
extern QueueHandle_t log_rq_handle;
extern SemaphoreHandle_t	up_alarm_lock_sem;
extern SemaphoreHandle_t	call_lock_sem;
#ifdef PLC
extern QueueHandle_t plc_msg_queue;
extern QueueHandle_t plc_data_queue;
#endif
#ifdef BRIDGE
extern QueueHandle_t plc_to_bridge_queue;
extern QueueHandle_t plc_fota_queue;
#endif

extern EventGroupHandle_t hbbeventgroup;
extern EventGroupHandle_t PowerKeyEventGroup;

extern SemaphoreHandle_t rf_message_sem;
extern SemaphoreHandle_t hb_task_sem;

#define HBB_GETLIST_EVENT 	(1 << 2)
#define HTTP_QUEUE_EVENT 	(1 << 1)//it means state of queue. 0:empty 1:not empty
