...
/*----------------------------------------------------------*/
	...
	TaskStatus_t	userPrvTaskStatusArray[20];
	uint32_t 		userPrvlTotalTime;
	uint32_t		userPrevidlecount;
	void vTaskGetRunTimeStats1( char *pcWriteBuffer )
	{
	TaskStatus_t *pxTaskStatusArray;
	volatile UBaseType_t uxArraySize, x, y;
	uint32_t ulTotalTime, ulStatsAsPercentage, ulidlePercentage;
	TaskHandle_t idletask = xTaskGetIdleTaskHandle();
	TCB_t * idleTCB;

	idleTCB = (TCB_t *)idletask;
		...
		*pcWriteBuffer = 0x00;
		...
		if( pxTaskStatusArray != NULL )
		{
			...
			ulTotalTime /= 100UL;
			ulidlePercentage = (idleTCB->ulRunTimeCounter - userPrevidlecount) / (ulTotalTime - userPrvlTotalTime);

			/* Avoid divide by zero errors. */
			if( (ulTotalTime > 0) && (ulidlePercentage < 10))
			{
				/* Create a human readable table from the binary data. */
				for( x = 0; x < uxArraySize; x++ )
				{
					...
					{
						if (0 == memcmp(pxTaskStatusArray[ x ].pcTaskName, userPrvTaskStatusArray[ y ].pcTaskName, configMAX_TASK_NAME_LEN))
						{
							ulStatsAsPercentage = (pxTaskStatusArray[ x ].ulRunTimeCounter - userPrvTaskStatusArray[ y ].ulRunTimeCounter) / (ulTotalTime - userPrvlTotalTime);
							...
							if( ulStatsAsPercentage > 0UL )
							{
								...
								#else
								{
									...
									sprintf( pcWriteBuffer, "\t%u\t\t%u%%\r\n", ( unsigned int ) (pxTaskStatusArray[ x ].ulRunTimeCounter - userPrvTaskStatusArray[ y ].ulRunTimeCounter), ( unsigned int ) ulStatsAsPercentage );
								}
								#endif
							}
							else
							{
								...
								#else
								{
									/* sizeof( int ) == sizeof( long ) so a smaller
									printf() library can be used. */
									sprintf( pcWriteBuffer, "\t%u\t\t<1%%\r\n", ( unsigned int ) (pxTaskStatusArray[ x ].ulRunTimeCounter - userPrvTaskStatusArray[ y ].ulRunTimeCounter) );
								}
								#endif
							}
			...
			userPrvlTotalTime = ulTotalTime;
			userPrevidlecount = idleTCB->ulRunTimeCounter;
			memcpy(userPrvTaskStatusArray, pxTaskStatusArray, uxCurrentNumberOfTasks * sizeof( TaskStatus_t ) );
